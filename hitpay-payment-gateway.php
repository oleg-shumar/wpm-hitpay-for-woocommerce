	...
	class WC_HitPay extends WC_Payment_Gateway {
			...
			if ( $order->get_payment_method() == $this->id ) {
				...
				if ( ! empty( $payment_request_id ) ) {
					...
					if ( empty( $payment_method ) ) {
						try {
							$hitpay_client = new Client( $this->api_key, $this->getMode() );
			...
			try {
				...
				if ( Client::generateSignatureArray( $salt, $data ) == $_POST['hmac'] ) {
					...
					if ( ! $HitPay_is_paid ) {
						...
						if ( $status == 'completed' && $order_total = $order->get_total() == $_POST['amount'] && $order_id == $_POST['reference_number'] && $order_data['currency'] == $_POST['currency'] ) {
		...
		public function process_payment( $order_id ) {
			...
			try {
				...
				$create_payment_request->setAmount( $order_total )->setCurrency( $order_data['currency'] )->setReferenceNumber( $order_id )->setWebhook( $webhook )->setRedirectUrl( $redirect_url )->setChannel( 'api_woocomm' );
